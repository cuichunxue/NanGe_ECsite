...
**書き換え漏れが無いかは、次のコマンドで確かめられます。**

```bash
npm run check:launch
```

【 】の穴埋めやOGPの仮ドメインが1つでも残っていれば、場所を列挙して終了コード1で止まります。バックエンドの設定(.env)は起動時に自動で確認されますが、購入者向けサイトは静的なHTMLをそのまま配るため、この確認をしないと「特定商取引法に基づく表示に【氏名を入力してください】が残ったまま公開」という事故が起きます。

**発送までの期間と返品条件は、特定商取引法に基づく表示と利用規約の両方に書く欄があります。** 必ず同じ内容にしてください。食い違うと、購入者との間で「どちらが正しいのか」というトラブルの元になります。
...
代金引換は、配送業者が店主に代引手数料（ヤマト運輸・佐川急便で1万円以下なら¥330）を請求します。購入者から頂かないと1件ごとに店主の持ち出しになるため、代金引換を選んだ注文には**¥330を加算**しています（ご注文手続き・注文詳細・領収書・メールのすべてに「代引手数料」として表示され、消費税は標準税率で内訳に含まれます）。

金額を変える・頂かない（0にする）ときは、次の2ファイルの `fee` を**同じ値に**してください。`frontend/legal.html`（商品代金以外の必要料金）と `frontend/terms.html`（第6条）の記載も合わせます。

- `backend/src/config/payment.ts`（注文時に確定する金額）
- `frontend/assets/js/payment.js`（画面に表示する金額）
...
### 商品の値付けで気をつけること（景品表示法）

商品登録の「元の価格」に金額を入れると、打ち消し線つきの元値と「-◯%」の値引き率が表示されます。**実際にその価格で販売していた実績が無い金額を「元の価格」にすることは、景品表示法の不当表示（有利誤認）にあたります。** 消費者庁の考え方では、比較する元値は「最近相当期間（目安として直近8週間のうち過半）にその価格で販売していた」ものである必要があります。値引きの演出のために架空の元値を入れないでください。管理画面の入力欄にも同じ注意を出しています。
...
ダッシュボードの「累計売上」は、**支払い済み・発送済み・受取完了の注文の税込合計**です。次のものは差し引かれていません。

| 項目 | 目安 | どこで分かるか |
|---|---|---|
| 決済代行の手数料 | KOMOJUはクレジットカードで概ね3.6%前後（契約による） | KOMOJUの管理画面 |
| 配送業者へ支払う送料 | 購入者から頂く送料と一致するとは限らない（送料無料の注文は全額が店主負担） | ダッシュボードの「常に送料無料」の注意書き、各注文の送料 |
| 代引手数料 | 購入者から¥330を頂き、配送業者へ支払う（差し引き0が目安） | 各注文の「代引手数料」 |
| 消費税 | 税込売上の約9.1%（10%対象）・約7.4%（8%対象） | 各注文の領収書に税率ごとの内訳 |
| 仕入れ・材料費 | このサイトは管理していない | 自分の帳簿 |

**確定申告のために注文を書き出すには**、データベースから直接CSVにできます（管理画面に書き出し機能はありません）。

```bash
psql "$DATABASE_URL" -c "\copy (SELECT \"orderNo\", status, \"paymentMethod\", subtotal, \"shippingFee\", \"codFee\", \"totalAmount\", \"paidAt\", \"createdAt\" FROM \"Order\" WHERE status IN ('PAID','SHIPPED','COMPLETED') ORDER BY \"paidAt\") TO 'orders.csv' CSV HEADER"
```

`DATABASE_URL` の末尾の `?schema=public` は psql が受け付けないため、`deploy/backup.sh` と同じように取り除いてから渡してください。
...
npm run check:launch            # 公開前の書き換え漏れ（【 】・仮ドメイン）を確認
